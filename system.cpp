				...
				if (i == j == -1) {
					for (int l = 0; l < layers_x + 1; ++l) {
						for (int k = 0; k < layers_y + 1; ++k)
							mol[t].FiSide[l * (layers_y + 2) + k] = geo.lambda_bb[l][k] * mol[t].G[l][k];
					}
				}
				else if (i == -1 && j == 0) {
					for (int l = 0; l < layers_x + 1; ++l) {
						for (int k = 0; k < layers_y + 1; ++k)
							mol[t].FiSide[l * (layers_y + 2) + k] = geo.lambda_bn[l][k] * mol[t].G[l][k];
					}
				}
				else if (i == -1 && j == 1) {
					for (int l = 0; l < layers_x + 1; ++l) {
						for (int k = 0; k < layers_y + 1; ++k)
							mol[t].FiSide[l * (layers_y + 2) + k] = geo.lambda_bf[l][k] * mol[t].G[l][k];
					}
				}
				else if (i == 0 && j == -1) {
					for (int l = 0; l < layers_x + 1; ++l) {
						for (int k = 0; k < layers_y + 1; ++k)
							mol[t].FiSide[l * (layers_y + 2) + k] = geo.lambda_nb[l][k] * mol[t].G[l][k];
					}
				}
				else if (i == 0 && j == 0) {
					for (int l = 0; l < layers_x + 1; ++l) {
						for (int k = 0; k < layers_y + 1; ++k)
							mol[t].FiSide[l * (layers_y + 2) + k] = geo.lambda_nn[l][k] * mol[t].G[l][k];
					}
				}
				else if (i == 0 && j == 1) {
					for (int l = 0; l < layers_x + 1; ++l) {
						for (int k = 0; k < layers_y + 1; ++k)
							mol[t].FiSide[l * (layers_y + 2) + k] = geo.lambda_nf[l][k] * mol[t].G[l][k];
					}
				}
				else if (i == 1 && j == -1) {
					for (int l = 0; l < layers_x + 1; ++l) {
						for (int k = 0; k < layers_y + 1; ++k)
							mol[t].FiSide[l * (layers_y + 2) + k] = geo.lambda_fb[l][k] * mol[t].G[l][k];
					}
				}
				else if (i == 1 && j == 0) {
					for (int l = 0; l < layers_x + 1; ++l) {
						for (int k = 0; k < layers_y + 1; ++k)
							mol[t].FiSide[l * (layers_y + 2) + k] = geo.lambda_fn[l][k] * mol[t].G[l][k];
					}
				}
				else if (i == 1 && j == 1) {
					for (int l = 0; l < layers_x + 1; ++l) {
						for (int k = 0; k < layers_y + 1; ++k)
							mol[t].FiSide[l * (layers_y + 2) + k] = geo.lambda_ff[l][k] * mol[t].G[l][k];
					}
				}
			}
...
void System::FindFiSolv() {
	...
	for (int i = 1; i < layers_x + 1; ++i) {
		for (int j = 1; j < layers_y + 1; ++j)
			fi_solv[i][j]= exp(-u_solv[i*(layers_y + 2) + j]);
	}
...
void System::FindFiTotal() {
	...
	for (int i = 1; i < layers_x + 1; ++i) {
		for (int j = 1; j < layers_y + 1; ++j)
			fi_total[i][j] = mol[1].fi[i][j] + mol[2].fi[i][j] + fi_solv[i][j];//?????
	}
}
...
void System::FindLagrangeMultipliers(int t) {
	...
	for (int i = 1; i < layers_x + 1; ++i) {
		for (int j = 1; j < layers_y + 1; ++j)
			mol[t].fi[i][j] = mol[t].fi[i][j] * chi_seg;
	}

	for (int i = 1; i < layers_x + 1; ++i) {
		for (int j = 1; j < layers_y + 1; ++j)
			fi_solv[i][j] = (fi_solv[i][j] - A[i][j])*mol[t].chi;
	}
	...
	c = Multiply_matrixes();
		...
		for (int i = 1; i < layers_x + 1; ++i) {
			for (int j = 1; j < layers_y + 1; ++j)
				mol[t].multipliers[i][j] = mol[t].u[i*(layers_y+2)+j]-c[]
		}
...
vector < vector < double > > System::Multiply_matrixes(vector < vector < double > > A, vector < vector < double > > U) {
